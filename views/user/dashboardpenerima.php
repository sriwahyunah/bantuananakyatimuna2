...
// STATISTIK DASHBOARD
// =====================
$totalBantuan = mysqli_fetch_assoc(
    mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM bantuan")
)['jml'];

$totalPenerima = mysqli_fetch_assoc(
    mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM penerima")
)['jml'];

// Admin diambil dari tabel user dengan role = admin
$totalAdmin = mysqli_fetch_assoc(
    mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM user WHERE role='admin'")
)['jml'];

$totalUser = mysqli_fetch_assoc(
    mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM user")
)['jml'];

$totalTransaksi = mysqli_fetch_assoc(
    mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM transaksi")
)['jml'];
...
$transaksiTerbaru = mysqli_query($koneksi, "
    ...
    FROM transaksi t
    LEFT JOIN bantuan b 
        ON b.id_bantuan = t.id_bantuan
    LEFT JOIN penerima p 
        ON p.id_penerima = t.id_penerima
